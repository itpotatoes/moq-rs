...
use crate::{frame_pts_us, snap_tick};
...
impl RunSlotClock {
    ...
    pub fn pc_identity(self, now_us: u64, fps: u64) -> Result<SourceIdentity, SlotError> {
        let slot = self.next_slot(now_us, fps)?;
        source_identity(slot, slot, frame_pts_us(slot, fps))
    }

    /// Essential haptic carries only the haptic tick snapped to the next PC
    /// frame. Its sequence remains the global 100-Hz source tick rather than a
    /// new 30-Hz local sequence.
    pub fn essential_haptic_identity(
        self,
        now_us: u64,
        fps: u64,
    ) -> Result<SourceIdentity, SlotError> {
        let pc_slot = self.next_slot(now_us, fps)?;
        let haptic_tick = snap_tick(pc_slot, fps);
        source_identity(haptic_tick, pc_slot, frame_pts_us(pc_slot, fps))
    }

    pub fn full_haptic_slot(self, now_us: u64) -> Result<u64, SlotError> {
        self.next_slot(now_us, 100)
    }
}
...
mod tests {
    ...
    fn common_clock_uses_next_global_slot_without_reset() {
        ...
        assert_eq!(switched.pts_us, frame_pts_us(154, 30));
    ...
    fn essential_haptic_keeps_global_tick_and_exact_pc_identity() {
        ...
        let haptic = clock.essential_haptic_identity(3_345_678, 30).unwrap();
        assert_eq!(haptic.pts_us, pc.pts_us);
        assert_eq!(haptic.event_id, pc.event_id);
        assert_eq!(haptic.slot, snap_tick(pc.slot, 30));
        assert_eq!(haptic.seq as u64, haptic.slot);
        assert_eq!(clock.full_haptic_slot(3_345_678).unwrap(), 235);
    }
