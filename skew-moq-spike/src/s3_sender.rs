...
use crate::{
    frame_pts_us, now_us, pack_header, snap_tick, JsonlLogger, HAPTIC_SAMPLES_PER_TICK,
    HAPTIC_TICK_US, HAPTIC_TIER_FULL, HDR, TRACK_HAPTIC, TRACK_PC,
};
...
impl SenderContext {
    pub fn validate(&self) -> anyhow::Result<()> {
        if self.fps == 0 {
            bail!("S3 fps must be greater than zero");
        }
...
fn haptic_payload(context: &SenderContext, tick: u64) -> &[u8] {
    let tick_bytes = HAPTIC_SAMPLES_PER_TICK * 2;
    let ticks = (context.haptic_pcm.len() / tick_bytes).max(1) as u64;
    let offset = ((tick % ticks) as usize) * tick_bytes;
    &context.haptic_pcm[offset..(offset + tick_bytes).min(context.haptic_pcm.len())]
}
...
fn exact_frame_for_tick(tick: u64, fps: u64) -> Option<u64> {
    // At 30 fps every 100-Hz tick maps to at most one PC frame. Check a small
    // integer neighborhood to avoid floating point and preserve snap_tick().
    let estimate = tick.saturating_mul(fps) / 100;
    let begin = estimate.saturating_sub(2);
    let end = estimate.saturating_add(3);
    (begin..=end).find(|&frame| snap_tick(frame, fps) == tick)
}
...
async fn produce_haptic(
    ...
    if essential {
        ...
        loop {
            let pts_us = frame_pts_us(frame, context.fps);
            let target_us = context.clock.anchor_us.saturating_add(pts_us);
            if target_us >= context.end_us {
                break;
            }
            ...
            let tick = snap_tick(frame, context.fps);
        ...
        let mut tick = context
            .clock
            .full_haptic_slot(now_us())
            .map_err(|error| anyhow!("derive full haptic slot: {error:?}"))?;
        loop {
            let nominal_pts = tick
                .checked_mul(HAPTIC_TICK_US)
                .context("full haptic PTS overflow")?;
            ...
            let (pts_us, event_id) = match exact_frame_for_tick(tick, context.fps) {
                Some(frame) => (
                    frame_pts_us(frame, context.fps),
...
mod tests {
    ...
    #[test]
    fn full_haptic_exact_anchors_use_the_frozen_snap_rule() {
        for frame in 0..300 {
            let tick = snap_tick(frame, 30);
            assert_eq!(exact_frame_for_tick(tick, 30), Some(frame));
        }
        assert_eq!(exact_frame_for_tick(1, 30), None);
    }
